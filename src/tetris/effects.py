...
class EffectsManager:
    ...
    def add_explosion_effect(self, grid_x, grid_y):
        """在指定网格位置添加爆炸特效 - 基于pyglet_demo的改进版本
        ...
        # 转换网格坐标到屏幕坐标
        screen_x = BORDER_WIDTH + grid_x * CELL_SIZE + CELL_SIZE // 2
        screen_y = BORDER_WIDTH + grid_y * CELL_SIZE + CELL_SIZE // 2
        
        # 创建爆炸粒子效果 - 使用pyglet_demo的颜色和逻辑
        particles = []
        particle_count = random.randint(20, 35)  # 随机粒子数量
        ...
        for _ in range(particle_count):
            # 随机角度和速度 - 全方向爆炸
            angle = random.uniform(0, 2 * math.pi)
            speed = random.uniform(80, 200)  # 适中的速度
            ...
            # 随机属性
            life_time = random.uniform(2.5, 4.5)  # 生命时间
            color = random.choice(explosion_colors)
            size = random.uniform(4, 10)  # 粒子大小
        ...
        explosion_effect = ExplosionEffect(particles)
